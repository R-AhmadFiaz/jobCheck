import { useState, useRef, useEffect, type ReactNode, type ButtonHTMLAttributes, type InputHTMLAttributes, type TextareaHTMLAttributes } from "react"

// ─── Types ────────────────────────────────────────────────────────────────────
export type RiskLevel = "safe" | "low" | "medium" | "high" | "critical"
export type Page =
  | "landing" | "login" | "register"
  | "dashboard" | "analyze" | "results" | "knowledge" | "admin" | "profile"
...
export function Input({
  label, helper, error, icon, iconRight, onIconRightClick, className = "", ...props
}: InputProps) {
  return (
    <div className="flex flex-col gap-1.5">
      ...
      >
        {icon && (
          <span className="absolute left-3 text-[var(--muted-foreground)] pointer-events-none">{icon}</span>
        )}
        <input
...
export function Alert({ variant = "info", title, children, onDismiss }: { variant?: AlertVariant; title?: string; children: ReactNode; onDismiss?: () => void }) {
  const styles: Record<AlertVariant, { bg: string; border: string; icon: string }> = {
    info:    { bg: "bg-blue-50 dark:bg-blue-950/40",   border: "border-blue-200 dark:border-blue-800",   icon: "text-blue-500" },
    success: { bg: "bg-emerald-50 dark:bg-emerald-950/40", border: "border-emerald-200 dark:border-emerald-800", icon: "text-emerald-500" },
    warning: { bg: "bg-amber-50 dark:bg-amber-950/40", border: "border-amber-200 dark:border-amber-800", icon: "text-amber-500" },
    error:   { bg: "bg-red-50 dark:bg-red-950/40",     border: "border-red-200 dark:border-red-800",     icon: "text-red-500" },
  }
  ...
  return (
    <div className={`flex gap-3 p-4 rounded-xl border ${s.bg} ${s.border}`}>
      ...
      {onDismiss && (
        <button onClick={onDismiss} className="flex-shrink-0 text-[var(--muted-foreground)] hover:text-[var(--foreground)] transition-colors">
          <XIcon size={16} />
        </button>
      )}
    </div>
...
// ─── Progress ─────────────────────────────────────────────────────────────────
export function Progress({ value, max = 100, variant = "primary", className = "" }: { value: number; max?: number; variant?: "primary" | "safe" | "warning" | "danger"; className?: string }) {
...
export function Tooltip({ children, content, side = "top" }: { children: ReactNode; content: string; side?: "top" | "bottom" | "left" | "right" }) {
  ...
  return (
    ...
    >
      {children}
      {visible && (
        <span className={`absolute z-50 pointer-events-none whitespace-nowrap px-2.5 py-1.5 bg-[var(--foreground)] text-[var(--background)] text-xs font-medium rounded-lg shadow-lg animate-fade-in ${positions[side]}`}>
          {content}
        </span>
      )}
    </span>
...
export function Avatar({ name, src, size = "md" }: { name: string; src?: string; size?: "sm" | "md" | "lg" | "xl" }) {
  ...
  return (
    <div className={`${sizes[size]} ${color} text-white font-semibold rounded-full flex items-center justify-center flex-shrink-0`} style={{ fontFamily: "var(--font-display)" }}>
      {initials}
    </div>
  )
}
...
export function Modal({ open, onClose, title, children, size = "md" }: { open: boolean; onClose: () => void; title?: string; children: ReactNode; size?: "sm" | "md" | "lg" }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className={`relative w-full ${widths[size]} bg-[var(--card)] rounded-2xl shadow-2xl border border-[var(--border)] animate-scale-in`}>
        {title && (
...
export function ToastContainer() {
  ...
  return (
    <div className="fixed bottom-4 right-4 z-[100] flex flex-col gap-2">
      {toasts.map(t => (
        <div key={t.id} className={`flex items-center gap-3 px-4 py-3 bg-[var(--card)] rounded-xl shadow-lg ${colors[t.variant]} animate-slide-right min-w-[260px] max-w-[340px]`}>
          {icons[t.variant]}
...
export function Pagination({ page, total, perPage, onChange }: { page: number; total: number; perPage: number; onChange: (p: number) => void }) {
  ...
  return (
    <div className="flex items-center gap-1">
      <button onClick={() => onChange(page - 1)} disabled={page <= 1} className="p-1.5 rounded-lg hover:bg-[var(--muted)] disabled:opacity-40 disabled:cursor-not-allowed transition-colors text-[var(--muted-foreground)] hover:text-[var(--foreground)]">
      ...
      <button onClick={() => onChange(page + 1)} disabled={page >= pages} className="p-1.5 rounded-lg hover:bg-[var(--muted)] disabled:opacity-40 disabled:cursor-not-allowed transition-colors text-[var(--muted-foreground)] hover:text-[var(--foreground)]">
...
// ─── Stat Card ────────────────────────────────────────────────────────────────
export function StatCard({ label, value, delta, icon, color = "primary" }: { label: string; value: string | number; delta?: string; icon?: ReactNode; color?: "primary" | "safe" | "warning" | "danger" }) {
  const colors = {
    ...
    <Card className="animate-fade-up">
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="text-3xl font-bold text-[var(--foreground)]" style={{ fontFamily: "var(--font-display)" }}>{value}</p>
