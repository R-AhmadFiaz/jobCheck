...
export function DashboardLayout() {
  ...
  const currentLabel = navItems.find((item) => item.to === location.pathname)?.label ?? 'JobCheck';
      ...
      <nav className="flex-1 overflow-y-auto px-3 py-4 space-y-0.5">
        {navItems.map((item) => (
          <NavLink
            ...
            className={({ isActive }) =>
              `sidebar-link w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-150 ${
                isActive
