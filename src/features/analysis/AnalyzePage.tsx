...
import { consumePendingPrefill } from '@/lib/pendingPrefill';
...
export function AnalyzePage() {
  ...
  // Next.js migration: React Router's `location.state` (read synchronously
  // during the initial render) has no equivalent here — reading
  // sessionStorage during the initializer would mismatch what the server
  // rendered. Consumed in an effect on mount instead (see below); this
  // feature is currently unused by any call site either way (see
  // useGoToAnalyzer.ts), so the one-render delay has no visible effect today.
  ...
  const mutation = useMutation({
    ...
    onSuccess: (result) => {
      ...
      router.push(`/analyses/${result.analysis._id}`);
    },
  });

  useEffect(() => {
    const prefill = consumePendingPrefill();
    // sessionStorage isn't available during SSR, so this can't run in the
    // initializer — it has to be an effect, and the only sink for what it
    // reads is this component's own state.
    // eslint-disable-next-line react-hooks/set-state-in-effect
    if (prefill) setText(prefill);
  }, []);

  // Pre-existing pattern, unchanged by the migration: drives a simulated
  // progress bar for the duration of the mutation (there's no real
  // upload/analysis progress signal from the API to hook into instead).
  useEffect(() => {
