...
import { checkRegisterRateLimit } from '@/lib/rateLimit/rateLimit';
...
export const POST = apiHandler(async (request: NextRequest) => {
  await checkRegisterRateLimit(getClientIp(request));
  await connectDB();
  const input = validateData(registerSchema, await readJsonBody(request));
  const { user, accessToken, refreshToken } = await authService.registerUser(
    input,
    getRequestMeta(request),
  );

  const response = NextResponse.json(successBody({ user, accessToken }), { status: 201 });
